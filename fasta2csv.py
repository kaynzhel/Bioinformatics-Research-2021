...
This code only prints out the Accession ID, Accession Description, and the Sequence from the fasta file.
...
def main():
    ...
    with open(file_name[:dot_index] + '.csv', mode='w') as csv_file:
        field_names = ['Accession ID', 'Accession Description', 'Sequence'] # header
        writer = csv.DictWriter(csv_file, fieldnames=field_names)
        writer.writeheader()
        for seq_record in SeqIO.parse(file_name, "fasta"): # parser
            row = {'Accession Description': seq_record.id, 'Accession Description': seq_record.description, 'Sequence': seq_record.seq}
            writer.writerow(row)
